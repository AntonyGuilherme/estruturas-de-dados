...
// alocando a memória para o uso da matriz
  alocarDinamicamenteAMatriz(mat->m,mat->tamy,mat->tamx);
  imprimeMatriz(mat);
...
void alocarDinamicamenteAMatriz(double ** matriz,int tamy,int tamx){
    ...
    /*
      ...
      # ... vetor a ser preenchido
    */
    matriz = malloc(tamx * sizeof(double*));
    ...
    */
    for(int i = 0 ; i < tamx ; i++){
        matriz[i] = malloc(tamy * sizeof(double));
    }
...
// Saida: a
{
  ...
  double ** transposta_de_a = NULL ;

   printf("Alocou");

  alocarDinamicamenteAMatriz(transposta_de_a,a->tamx,a->tamy);
  ...
  for (i=0; i < a->tamx; i++){
    for(j=0; j < a->tamy; j++){
      ...
      transposta_de_a[j][i] = a->m[i][j];

      escreveMemLog((long int)(&(a->m[i][j])),sizeof(double));
      escreveMemLog((long int)(&(transposta_de_a[j][i])),sizeof(double));
    }
  }

  // desalocando a matriz para substitui-la pela transposta
  desalocarAMatriz(a->m,a->tamy);
...
// Saida: a
{
  ...
  erroAssert(((a->tamx>0)&&(a->tamy>0)),"Matriz já foi destruida");
